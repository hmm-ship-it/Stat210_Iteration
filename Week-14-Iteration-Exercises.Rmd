...
```{r}
exams %>% map_lgl(~length(.) == 10) #returns vector, instead of a list
 #map_lgl(bind_rows)
```
...
```{r}
exams %>% map_dfr(~sum(.)) -> aSum
exams %>% map_dfr(~min(.)) -> aMin
final <- ((aSum - aMin) / 9)
head(final)

#OR this is how the teacher did it... in one line
exams %>% map_dfr(~(sum(.) - min(.))/9) -> clean_exam
...
```{r}
exams %>% map2_dbl(extra_credit, ~mean(.x) + .y)
map2_dbl(exams, extra_credit, ~mean(.x) + .y)
```
